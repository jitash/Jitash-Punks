const JitashPunks = artifacts.require("./JitashPunks")
...
contract('JitashPunks', ([deployer, user]) => {
    ...
    describe('Deployment', () => {
        ...
        beforeEach(async () => {
            ...
            jitashPunks = await JitashPunks.new(
                NAME,
                SYMBOL,
                COST,
                MAX_SUPPLY,
        ...
        it('Returns the contract name', async () => {
            result = await jitashPunks.name()
            result.should.equal(NAME)
        })

        it('Returns the symbol', async () => {
            result = await jitashPunks.symbol()
            result.should.equal(SYMBOL)
        })

        it('Returns the cost to mint', async () => {
            result = await jitashPunks.cost()
            result.toString().should.equal(COST.toString())
        })

        it('Returns the max supply', async () => {
            result = await jitashPunks.maxSupply()
            result.toString().should.equal(MAX_SUPPLY.toString())
        })

        it('Returns the max mint amount', async () => {
            result = await jitashPunks.maxMintAmount()
            result.toString().should.equal('1')
        })

        it('Returns the time deployed', async () => {
            result = await jitashPunks.timeDeployed()
        ...
        it('Returns the amount of seconds from deployment to wait until minting', async () => {
            ...
            result = await jitashPunks.allowMintingAfter()
        ...
        it('Returns how many seconds left until minting allowed', async () => {
            let buffer = 2
            let target = Number(milliseconds.toString().slice(0, 3))
            result = await jitashPunks.getSecondsUntilMinting()
            result = Number(result)
    ...
    describe('Minting', async () => {
        describe('Success', async () => {
            ...
            beforeEach(async () => {
                ...
                result = await jitashPunks.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') })
            })
            ...
            it('Updates the total supply', async () => {
                result = await jitashPunks.totalSupply()
                result.toString().should.equal('1')
            })

            it('Returns IPFS URI', async () => {
                result = await jitashPunks.tokenURI(1)
                result.should.equal(`${IPFS_IMAGE_METADATA_URI}1.json`)
            })

            it('Returns how many a minter owns', async () => {
                result = await jitashPunks.balanceOf(user)
                result.toString().should.equal('1')
            })

            it('Returns the IDs of minted NFTs', async () => {
                result = await jitashPunks.walletOfOwner(user)
        ...
        describe('Failure', async () => {
            ...
            it('Attempt to mint before mint date', async () => {
                await jitashPunks.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') }).should.be.rejectedWith(EVM_REVERT)
            })
        })
    })
    ...
    describe('Updating Contract State', async () => {
        describe('Success', async () => {
            ...
            it('Sets the cost', async () => {
                ...
                await jitashPunks.setCost(cost, { from: deployer })
                result = await jitashPunks.cost()
            ...
            it('Sets the pause state', async () => {
                ...
                await jitashPunks.setIsPaused(isPaused, { from: deployer })
                result = await jitashPunks.isPaused()
            ...
            it('Sets the reveal state', async () => {
                ...
                await jitashPunks.setIsRevealed(isRevealed, { from: deployer })
                result = await jitashPunks.isRevealed()
            ...
            it('Sets the max batch mint amount', async () => {
                ...
                await jitashPunks.setmaxMintAmount(5, { from: deployer })
                result = await jitashPunks.maxMintAmount()
            ...
            it('Sets the IPFS not revealed URI', async () => {
                ...
                await jitashPunks.setNotRevealedURI(uri, { from: deployer })
                result = await jitashPunks.notRevealedUri()
            ...
            it('Sets the base extension', async () => {
                ...
                await jitashPunks.setBaseExtension('.example', { from: deployer })
                result = await jitashPunks.baseExtension()
