...
// Import ABI + Config
import JitashPunks from '../abis/JitashPunks.json'
import config from '../config.json'

function App() {
	const [web3, setWeb3] = useState(null)
	const [jitashPunks, setOpenPunks] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const jitashPunks = new _web3.eth.Contract(JitashPunks.abi, JitashPunks.networks[_networkId].address)
			setOpenPunks(jitashPunks)

			const maxSupply = await jitashPunks.methods.maxSupply().call()
			const totalSupply = await jitashPunks.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await jitashPunks.methods.allowMintingAfter().call()
			const timeDeployed = await jitashPunks.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await jitashPunks.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
			} else {
				setOwnerOf([])
			}
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (jitashPunks && account) {
			setIsMinting(true)
			setIsError(false)

			await jitashPunks.methods.mint(1).send({ from: account, value: 0 })
				.on('confirmation', async () => {
					const maxSupply = await jitashPunks.methods.maxSupply().call()
					const totalSupply = await jitashPunks.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await jitashPunks.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
				...
				<section id='welcome' className='welcome'>
					...
					<Row className='header my-3 p-3 mb-0 pb-0'>
						<Col xs={12} md={12} lg={8} xxl={8}>
							<h1>Open Punks</h1>
							<p className='sub-header'>Availble on 08 / 15 / 22</p>
						</Col>
						<Col className='flex social-icons'>
							<a
								href="https://twitter.com/jitash_cn"
								target='_blank'
				...
				<section id='about' className='about'>
								...
								<div>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${openseaURL}/assets/${jitashPunks._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row style={{ marginTop: "100px" }}>
						<Col>
							{jitashPunks &&
								<a
									href={`${explorerURL}/address/${jitashPunks._address}`}
									target='_blank'
									className='text-center'>
									{jitashPunks._address}
								</a>
							}
						</Col>
					</Row>
